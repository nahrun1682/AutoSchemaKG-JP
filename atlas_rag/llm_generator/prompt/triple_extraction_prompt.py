TRIPLE_INSTRUCTIONS = {
    ...
    "ja": {
        "system": "あなたは常に有効なJSONオブジェクト配列のみで回答し、一切の説明や補足テキストを追加しない丁寧なアシスタントです。",
        "entity_relation": """与えられた文章から重要なエンティティとそれらの関係を漏れなく抽出し、簡潔にまとめてください。関係はエンティティ間のつながりを端的に説明し、HeadやTailの情報を繰り返してはいけません。エンティティは可能な限り具体的な名詞を用い、代名詞は除外してください。JSON以外の文字列を出力することは禁止されています。
        必ず**以下のJSON形式**で出力し、値は日本語で記述してください:\n
        [
            {
                "Head": "{名詞}",
                "Relation": "{動詞または関係の要約}",
        ...
        "event_entity": """段落に含まれる各イベントと、それぞれに参加したエンティティを抽出してください。イベントは独立した単文として記述し、三点リーダーなどの省略記号は使用しないでください。列挙するエンティティは具体的な固有名詞または名詞句に限り、代名詞は除外してください。JSON以外のテキストは出力しないでください。
        必ず**以下のJSON形式**で出力し、値は日本語で記述してください:\n
        [
            {
                "Event": "{イベントを説明する簡潔な文}",
                "Entity": ["エンティティ1", "エンティティ2", "..."]
            }...
        ]
        以下の文章を解析してください：""",
       
        "event_relation": """段落の中に含まれるイベント同士の時系列または因果の関係をまとめてください。各イベントは独立した単文とし、関係タイプは「before」「after」「at the same time」「because」「as a result」のいずれかを必ず使用します。抽出される各トリプルは具体的かつ自己完結的である必要があります。省略記号は使用せず、JSON以外の文字列は出力しないでください。
        必ず**以下のJSON形式**で出力し、値は日本語で記述してください:\n
        [
            {
                "Head": "{イベント1を説明する簡潔な文}",
                "Relation": "{上記いずれかの関係タイプ}",
                "Tail": "{イベント2を説明する簡潔な文}"
            }...
        ]
        以下の文章を解析してください：""",
...
CONCEPT_INSTRUCTIONS = {
    ...
    "ja": {
        "event": """イベントを1つ提示します。そのイベントを抽象化した1～2語のフレーズを複数個作成してください。
            回答は必ず次の形式にしてください：フレーズ1, フレーズ2, フレーズ3,...
            ここに記載した以外の文章や記号を追加してはいけません。回答はすべて日本語で記述してください。
            抽象化フレーズは以下を満たす必要があります。
            1. イベントの種類や関連概念を的確に表現していること
            2. 指定フォーマットを厳守し、余分な文字列を含めないこと
            3. 可能であれば抽象度の異なる語を3つ以上提示すること
            4. 同一語や入力文中の語を繰り返さないこと
            5. これ以上挙げられない場合は直ちに終了し、解説は不要です

            イベント：地方自治体が総合防災計画を公開する
            あなたの回答：防災策, 行政施策, 住民保護, 記者会見
            イベント：研究チームが新薬の臨床結果を発表する
            あなたの回答：臨床発表, 医療研究, 新薬開発, データ公開
            イベント：製造会社が海外工場の操業停止を決定する
            あなたの回答：操業停止, リスク管理, 供給網, 企業声明
            イベント：[EVENT]
            あなたの回答：""",
        "entity":"""エンティティとそのコンテキストを提示します。そのエンティティを抽象化した1～2語のフレーズを複数個作成してください。
            回答は必ず次の形式にしてください：フレーズ1, フレーズ2, フレーズ3,...
            ここに記載した以外の文章や記号を追加してはいけません。回答はすべて日本語で記述してください。
            抽象化フレーズは以下を満たす必要があります。
            1. エンティティの種類や関連概念を適切に表すこと（タイプ名詞や代表的概念を推奨）
            2. 指定フォーマットを厳守し、余分な文字列を含めないこと
            3. 可能であれば異なる抽象度の語を3つ以上提示すること
            4. 同一語や入力文中の語を繰り返さないこと
            5. これ以上挙げられない場合は直ちに終了し、解説は不要です

            ENTITY: ソニーグループ
            CONTEXT: 音響機器と映画事業の双方で新製品を発表した
            あなたの回答：テクノロジー企業, 家電メーカー, グローバルブランド, 上場企業

            ENTITY: 東京証券取引所プライム市場
            CONTEXT: 新たに3社の上場承認を発表した
            あなたの回答：株式市場, 金融インフラ, 取引所区分, 上場基準

            ENTITY: 地方創生ファンド
            CONTEXT: 地域交通スタートアップへの出資を決定した
            あなたの回答：投資ビークル, 公民連携, 地域支援, ファンド

            ENTITY: [ENTITY]
            CONTEXT: [CONTEXT]
            あなたの回答：""",
        "relation":"""関係語を1つ提示します。その関係を抽象化した1～2語のフレーズを複数個作成してください。
            回答は必ず次の形式にしてください：フレーズ1, フレーズ2, フレーズ3,...
            ここに記載した以外の文章や記号を追加してはいけません。回答はすべて日本語で記述してください。
            抽象化フレーズは以下を満たす必要があります。
            1. 入力の関係を代表するタイプや概念を示していること
            2. 指定フォーマットを厳守し、余分な文字列を含めないこと
            3. 可能であれば異なる抽象度の語を3つ以上提示すること
            4. 同一語や入力文中の語を繰り返さないこと
            5. これ以上挙げられない場合は直ちに終了し、解説は不要です
            ...
            RELATION: 資金提供する
            あなたの回答：投資, 支援, 融資, 財務協力
            RELATION: ともに開発する
            あなたの回答：共同研究, アライアンス, 共創, 技術連携
            RELATION: [RELATION]
            あなたの回答："""
    },
    "zh-CN": {
